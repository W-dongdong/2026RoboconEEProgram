...
DM3519::DM3519(CAN_HandleTypeDef* hcan, uint8_t TxID, uint16_t MST_ID) 
    : m_hcan(hcan), m_Error(0), m_Pos(0), m_Vel(0), m_Torque(0), 
	  m_T_MOS(0), m_T_Rotor(0), m_redRatio(3591.0f/187.0f),
	  m_TxID(TxID), m_MST_ID(MST_ID), m_SetSpeed(0), m_SetPos(0)
	{
	}
...
uint8_t DM3519::ParseFeedback(CanMsg* RxMsg)
{
	...
	else
	{
		...
		uint16_t raw_P = (RxMsg->Data[1] << 8) | RxMsg->Data[2];//待处理
		uint16_t raw_V = (RxMsg->Data[3] << 4) | (RxMsg->Data[4] >> 4);//返回最后一位是小数，比如105即10.5,正负代表方向
		uint16_t raw_T = ((RxMsg->Data[4] & 0xF) << 8) |  RxMsg->Data[5];//待处理
		m_T_MOS = RxMsg->Data[6];
		m_T_Rotor = RxMsg->Data[7];
		
		m_Pos = uint_to_float(raw_P, -5000, 5000, 16);
        m_Vel = uint_to_float(raw_V, -20, 20, 12);
        m_Torque = uint_to_float(raw_T, -10, 10, 12);
...
{	
	...
	TxMsg.ID = (uint32_t)(m_TxID + 0x200);
	TxMsg.IDE = CAN_ID_STD;
	TxMsg.RTR = CAN_RTR_DATA;
	TxMsg.DLC = 4;
...
float DM3519::uint_to_float(int x_int, float x_min, float x_max, int bits) {
